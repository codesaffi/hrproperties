...
export default function Signup() {
  ...
  if (!password || password.length < 8) return setError("Password must be at least 8 characters");
    try {
      ...
      const res = await fetch(`${backendUrl || "http://localhost:4000"}/api/user/register`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <form className="bg-white p-8 rounded-lg shadow-md w-full max-w-md" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-bold mb-6 text-blue-800 text-center">Sign Up</h2>
        <input
          type="text"
          placeholder="Name"
          value={name}
        ...
        <input
          ...
          required
        />
        <button type="submit" className="w-full bg-blue-800 text-white py-2 rounded-lg font-bold hover:bg-blue-900 transition">Sign Up</button>
