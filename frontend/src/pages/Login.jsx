...
export default function Login() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const res = await fetch(`${backendUrl || "http://localhost:4000"}/api/user/login`, {
				method: "POST",
		...
		<div className="min-h-screen flex items-center justify-center bg-gray-50">
			<form className="bg-white p-8 rounded-lg shadow-md w-full max-w-md" onSubmit={handleSubmit}>
				...
				<input
					...
					required
				/>
				<button type="submit" className="w-full bg-blue-800 text-white py-2 rounded-lg font-bold hover:bg-blue-900 transition">Login</button>
